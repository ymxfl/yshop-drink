	...
	<view class="container">
		...
		<modal :show="ensureAddressModalVisible" custom :mask-closable="false" :radius="'0rpx'" width="90%">
			<view class="modal-content">
				...
				<button type="primary" class="pay_btn" @tap="debouncedPay">确认并付款</button>
			</view>
		</modal>
...
const pay = async() => {
	...
	if (payType.value == 'yue' && parseFloat(member.value.nowMoney || 0) < parseFloat(amount.value)) {
		uToast.value.show({
			message: '余额不足',
			type: 'error'
		});
    ...
    if(amount.value == 0){
		...
		await balancePay(order);
		return
   }

	if (payType.value == 'weixin') {
		// 微信支付
		await weixinPay(order);
...
const balancePay = async(order) => {
	...
	member.value.nowMoney = (parseFloat(member.value.nowMoney || 0) - parseFloat(amount.value)).toFixed(2)
